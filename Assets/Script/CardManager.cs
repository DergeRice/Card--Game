...
public class CardManager : MonoBehaviour
{
    ...
    List<CardScript> InputedCardInfo = new List<CardScript>();
    ...
    {
        for (int i = 0; i < NetworkManager.networkManager.PlayersHand.Count; i++)
        {
    ...
    public void CalMySenctence()
    {
        ...
        Debug.Log(CheckMySentenceAvailable());
    }

    public int CheckMySentenceAvailable()
    {
        bool HasVerb = false ,HasNoun = false, PointAble = false, IsVerbNormal = false;
        ...
        {
            if (InputedCardInfo[i].PartOfSpeech == "동사" && i > 1) //가장먼저 동사부터 찾아야함
            {
                if(i == 1) return 1001;
        ...
        for (int i = 0; i < InputedCardInfo.Count; i++) // 나머지들 품사 구분
        {
            ...
            if(InputedCardInfo[i].CardType == "두")
            {
                ...
                bool check = false;
                if(i == 1)// Do 가 의문문일때는 뒤에 명사가 와야한다.
                {
            ...
            {
                bool check = false;
                if(i == 1)// 조동사 가 의문문일때는 뒤에 명사가 와야한다.
                {
                    for (int j = i; j < InputedCardInfo.Count; i++)
                    {   
        ...
        if(!HasVerb || !HasNoun) return 1015; // 둘중 하나라도 한개조차 없으면 false return;
